...
when_to_use: "review, relire le code, vérifier, y a-t-il un problème, faut-il optimiser, avant merge, regarder la PR, regarder le ticket, review my code, check changes, before merge, code review"
metadata:
  version: "3.14.0"
---
...
Activate when the user mentions: ticket, PR, "review all", triage, or "batch". Skip the diff flow and run this instead.

**Flow:**

1. Pull pending items: open Shortcut stories (the project ticket manager) and `gh pr list -R <repo> --state open`.
2. For each item, check if a fix already exists before analyzing:
   ...
   Three outcomes: already shipped (close with note), merged but unreleased (note "fixed, ships next release", close), or no fix (analyze).
3. Classify each item (bug / enhancement / chore / question) and map it to the matching Shortcut workflow state.
4. Draft every reply and show it to the user for confirmation before any Shortcut update or GitHub write operation (`gh pr comment` / `gh pr review`).
5. If no open items exist: shift to historical analysis. Review recently closed Shortcut stories and `gh pr list -R <repo> --state merged --limit 50` for recurring themes, fixes closed without resolution, and deferred work with demand. Produce a short summary with **Fix candidates** and **Feature candidates**. Do not implement without approval.
