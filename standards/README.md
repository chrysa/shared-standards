...
**Role.** The canonical chrysa standards corpus. `STANDARDS.chrysa.md` is the **source of
truth**; every agent view is generated from it. `annexes/` holds the normative detail that is
...
| Path                   | Purpose                                                              |
| ---------------------- | -------------------------------------------------------------------- |
| `STANDARDS.chrysa.md`  | the canonical socle — the **source of truth**, never distributed verbatim |
| `CORE.chrysa.md`       | GENERATED slim core (rule title + per-domain pointer) — the body `scripts/distribute-standards.sh` inlines into each repo's `CLAUDE.md` managed `chrysa:standards` block |
| `rules/`               | GENERATED per-domain detail (`<domain>.md`), loaded on demand — see `rules/README.md` |
| `rule-domains.yaml`    | the domain taxonomy that classifies each canon rule (drives the generator) |
| `annexes/`             | normative annexes, referenced by URL from the socle, never inlined    |
| `README.md`            | this file                                                             |

Views are generated by `scripts/gen_agent_views.py` (`make gen-agent-views`) and kept
byte-faithful by the `agent-views-drift` gate (pre-commit + CI). Edit the canon, then
regenerate — never hand-edit `CORE.chrysa.md` or `rules/`.
